...
public class FuncionarioMenu {
	...
	public static void main(String[] args){
		...
		while(!login_tentativa){
			...
			//validando login
			if(login_tentativa = FuncionarioMenu.login(scan)) {
				...
				while (true) {	
					...
					if (opcao == 1) {
						FuncionarioMenu.mostrarTodos();
					} else if (opcao == 2) {
						FuncionarioMenu.inserir(scan);
					} else if (opcao == 3) {
						FuncionarioMenu.atualizar(scan);
					} else if (opcao == 4) {
						FuncionarioMenu.excluir(scan);
					} else if (opcao == 5){
						FuncionarioMenu.sair(scan);
					}
				}
